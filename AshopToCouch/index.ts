...
import { AshopTrip, AshopCruise, AshopHaul, AshopSample, AshopHaulTypeName, AshopTripTypeName, AshopCruiseTypeName, AshopSampleSpecies, AshopSpecimen, AshopSampleTypeName, AshopSampleSpeciesTypeName, AshopSpecimenTypeName } from '../libs/bn-models/models/ashop';
import { AllHaulsSQL, strHaulSQL, strSamplesByHaulSQL, strSubSamplesByParentSQL, strTripSQL, AllCruisesSQL, strCruiseSQL, strFlowScaleWeightSQL, strMammalInteractionSQL, strMammalSpecimenSQL, strMammalSightingSQL, strBirdInteractionSQL, strBirdEventSQL, strBirdInteractOutcomeSQL, strSampleSpeciesSQL, strSpecimensSQL, strBiostructuresSQL, strAllSampledBySQL, strAllSpeciesSQL, strConditionLookupSQL } from './norpacsql';
import { Point } from 'geojson';
import { InteractionEvent, SightingEvent, InteractionEventTypeName, VesselStatusTypeName, SpeciesTypeName } from '../libs/bn-models';
import { AshopBiostructure, AshopBiostructureTypeName } from '../libs/bn-models/models/ashop/ashop-biostructure';
...
var strConnection: string = "(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=" + NorpacHost + ")(PORT=" + NorpacPort + "))(CONNECT_DATA =(SERVICE_NAME=" + NorpacServiceName + ")))"
...
function RemoveDocNullVals(Document: any){
  ...
  for (let item in Document){
    if (Document[item] == null || Document[item] == [] || Document[item] == [null]){
...
function DmsToDD(iDegrees: number, iMinutes: number, iSeconds: number, strEastOrWest: string) {
  if (strEastOrWest == 'W') {
...
async function BuildHaul(odb: any, iCruiseID: number, iHaulID: number, strPermit: string) {
  ...
  let docNewSamples: AshopSample[] = await BuildSamples(odb, iCruiseID, iHaulID, null, strPermit);
...
async function BuildSamples(odb: any, iCruiseID: number, iHaulID: number, iParentID: number, strPermit: string) {
  ...
  for (let i = 0; i < lstSampleData.length; i++) {
    let lstSubsamples: AshopSample[] = await BuildSamples(odb, iCruiseID, iHaulID, lstSampleData[i][1], lstSampleData[i][10]);
    ...
    let docNewSample: AshopSample = {
      _id: strCouchID,
      type: AshopSampleTypeName,
      createdBy: null, // todo
      createdDate: null, // todo - query history table, if not exists use DATE_OF_ENTRY
      updatedBy: null, // todo
      updatedDate: null, // todo, if history record exists, use DATE_OF_ENTRY, else null
...
//SELECT * FROM ATL_LOV_SPECIES_CODE WHERE SPECIES_CODE IN ( SELECT DISTINCT ATL_SPECIES_COMPOSITION.SPECIES_CODE FROM ATL_SPECIES_COMPOSITION JOIN ATL_SAMPLE ON ATL_SPECIES_COMPOSITION.CRUISE = ATL_SAMPLE.CRUISE AND ATL_SPECIES_COMPOSITION.PERMIT = ATL_SAMPLE.PERMIT AND ATL_SPECIES_COMPOSITION.SAMPLE_SEQ = ATL_SAMPLE.SAMPLE_SEQ JOIN ATL_HAUL ON ATL_SAMPLE.CRUISE = ATL_HAUL.CRUISE AND ATL_SAMPLE.PERMIT = ATL_HAUL.PERMIT AND ATL_SAMPLE.HAUL_SEQ = ATL_HAUL.HAUL_SEQ WHERE ATL_HAUL.DEPLOY_LATITUDE_DEGREES < 49)
